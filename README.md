...
[![Cursor SDK](https://img.shields.io/badge/%40cursor%2Fsdk-1.0.31-green)](https://www.npmjs.com/package/@cursor/sdk)
...
- 直接从任务回答的口语摘要生成播报；**不增加模型调用，也不朗读命令、代码块和工具过程**
...
- **口语摘要**：本地从任务回答开头提取并清理适合朗读的内容，不调用第二个 Agent
...
- 完整结果先显示为文字，再从任务回答的开场摘要中本地提取口语内容。代码、命令和详情留在文字中，工具过程不逐条朗读
- 口语通常为一到四句话，最多约 180 字。生成过程不调用第二个模型，因此没有额外 Token 消耗和等待
- 关闭播报时不会发声，也不会播报即时确认
...
| **结论播报** | 任务完成后 | 本地提取的开场摘要 | 不增加模型调用；失败/未验证等关键信息会保留 |
