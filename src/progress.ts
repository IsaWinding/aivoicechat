...
export function toSpeechText(text: string, maxChars = 180): string {
  const cleaned = text
    .replace(/(```|~~~)[\s\S]*?(?:\1|$)/g, "")
    .replace(/`[^`]*`/g, "")
    .replace(/!\[[^\]]*\]\([^)]*\)/g, "")
    .replace(/\[([^\]]+)\]\([^)]*\)/g, "$1")
    .replace(/^\s*(?:[$>]\s*)?(?:npm|npx|git|pip|curl|powershell|node|python|cd)\s+.*$/gm, "")
    .replace(/^\s*(?:\|.*|(?:const|let|var|import|export|function)\s+.*)$/gm, "")
  ...
  const suffix = "详细内容请看文字。";
  const cut = cleaned.slice(0, Math.max(0, maxChars - suffix.length));
  const lastStop = Math.max(cut.lastIndexOf("。"), cut.lastIndexOf("！"), cut.lastIndexOf("？"), cut.lastIndexOf("."));
  return `${lastStop >= 0 ? cut.slice(0, lastStop + 1) : cut}${suffix}`;
}

/** Prefer the model's opening summary, skipping headings; retain later failure caveats. */
export function replyForSpeech(text: string): string {
  const prose = text.replace(/(```|~~~)[\s\S]*?(?:\1|$)/g, "")
    .replace(/^\s{0,3}#{1,6}\s+.*$/gm, "");
  const paragraphs = prose.split(/\n\s*\n/).map(part => toSpeechText(part, 1000)).filter(Boolean);
  const summary = paragraphs[0] || "";
  const caveat = paragraphs.slice(1).flatMap(part => part.split(/(?<=[。！？!?])/))
    .find(part => /失败|未通过|未完成|尚未|无法|未验证|没有验证/.test(part));
  if (caveat && !summary.includes(caveat.trim())) {
    return toSpeechText(`${toSpeechText(summary, 100)} ${toSpeechText(caveat.trim(), 70)}`, 180);
  }
...
export function interpretSdkEvent(event: unknown): ProgressNotice | null {
  ...
  if (record.type === "tool_call") {
    ...
    if (status === "running") {
      ...
      return { text, speak: false, speakText: label.verbStart };
    }
    if (status === "error") {
      const text = `${name} 失败${targetText}`;
      return { text, speak: true, speakText: "操作遇到问题，请查看文字详情。" };
    }
  ...
  if (record.type === "task") {
    ...
    return { text, speak: false, speakText: summarizeForSpeech(text, 80) };
  }
