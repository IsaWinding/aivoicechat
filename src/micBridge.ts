...
export interface MicBridgeEvent {
  kind: "status" | "progress" | "assistant" | "error" | "hello" | "speak" | "mute" | "unmute" | "settings";
  text: string;
  state?: string;
  autoSpeak?: boolean;
}
...
export class MicBridge {
  ...
  broadcast(event: MicBridgeEvent): void {
    if (event.kind !== "speak" && event.kind !== "mute" && event.kind !== "unmute" && event.kind !== "settings") {
  ...
  private route(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    if (req.method === "POST" && (url.pathname === "/transcript" || url.pathname === "/state" || url.pathname === "/auto-speak")) {
      ...
      req.on("end", () => {
        try {
          const data = JSON.parse(body || "{}") as { text?: string; listening?: boolean; enabled?: boolean };
